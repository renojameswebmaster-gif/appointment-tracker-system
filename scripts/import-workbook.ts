...
const workbookPath =
  process.argv[2] ??
  "C:\\Users\\Reno James\\Downloads\\Appointment Tracker - PH (1).xlsx";
...
async function main() {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      const dateText = find(
        ...
        sheetName === "2026"
          ...
          : ["date of appointment", "date of appt", "appointment date"],
      );
      const appointmentDate = parseDate(dateText, sheetName);
      if (!appointmentDate) continue;
      const rawRow = Object.fromEntries(
        Object.entries(row).map(([key, cell]) => [key, text(cell)]),
      );
      const originalStatus = find(row, [
        "status",
        "status ",
        "appointment status",
      ]).toUpperCase();
      data.push({
        ...
        sdrName: find(row, ["sdr name", "appointment setter", "booked by"]),
        doctorName: find(row, [
          "name of doctor",
          "doctor's name",
          "doctor/provider",
  ...
  if (!data.length)
    throw new Error("No dated appointment rows were found in the workbook.");
  const result = await prisma.appointment.createMany({
    data,
    skipDuplicates: false,
  });
  console.log(
    `Imported ${result.count} appointment records from ${appointmentSheets.join(", ")}.`,
  );
  console.log(
    "Original workbook values are preserved in rawData; non-standard statuses were retained there and mapped to MISSED for dashboard totals.",
  );
}
